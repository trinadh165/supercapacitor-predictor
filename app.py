...
# --- Load the models ---
charge_model_xgb, discharge_model_xgb, feature_columns = load_all_data_and_train_models()
...
# ### NEW FEATURE: Create a FOUR-tabbed interface ###
tab1, tab2, tab3, tab4 = st.tabs(["Supercapacitor Predictor", "Technology Comparison", "Training Data", "Reference Datasets"])
...
# --- TAB 3: Training Data Viewer (Unchanged) ---
with tab3:
    # (The code for the training data viewer is unchanged)
    ...
    # We need to re-generate the large dataset here for display, but it's fast
    @st.cache_data
    def get_large_dataset_for_display():
        # This is a simplified version of the generation just for display
        csv_data = """
        Electrode_Material,Electrolyte_Type,Device_Type,Current_Density_Ag-1,Cycles_Completed,Charge_Capacity_mAh_g-1,Discharge_Capacity_mAh_g-1
        CuO/MnO2@MWCNT,RAE,Coin Cell,1.0,0,192.03,182.89
        CuO/MnO2@MWCNT,RAE,Coin Cell,1.0,5000,173.79,165.51
        ...
        """
        df_small = pd.read_csv(StringIO(csv_data))
        # Re-using the generation logic
        all_new_data = []
        unique_configs = df_small[['Electrode_Material', 'Electrolyte_Type', 'Device_Type', 'Current_Density_Ag-1']].drop_duplicates()
        for index, config_row in unique_configs.iterrows():
            group_df = df_small[(df_small['Electrode_Material'] == config_row['Electrode_Material']) & (df_small['Electrolyte_Type'] == config_row['Electrolyte_Type']) & (df_small['Device_Type'] == config_row['Device_Type']) & (df_small['Current_Density_Ag-1'] == config_row['Current_Density_Ag-1'])]
            start_row = group_df.loc[group_df['Cycles_Completed'].idxmin()]
            if len(group_df) > 1:
                end_row = group_df.loc[group_df['Cycles_Completed'].idxmax()]
                max_cycles, start_charge, end_charge, start_discharge, end_discharge = (end_row['Cycles_Completed'], start_row['Charge_Capacity_mAh_g-1'], end_row['Charge_Capacity_mAh_g-1'], start_row['Discharge_Capacity_mAh_g-1'], end_row['Discharge_Capacity_mAh_g-1'])
    ...
    df_training_display = get_large_dataset_for_display()
    st.dataframe(df_training_display)

# ### NEW FEATURE: A fourth tab for the reference datasets ###
with tab4:
    st.header("📖 Reference Datasets")
    st.markdown("This section provides the raw, cycle-by-cycle datasets for the battery technologies used in the comparison dashboard.")
